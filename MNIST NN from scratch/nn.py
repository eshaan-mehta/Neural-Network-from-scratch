...
#lists storing indices of all images model predicted 
correct_indices = []
incorrect_indices = []
...
def get_accuracy(predictions, labels, on_testing_data = False):
  total_correct = 0
  ...
  for i, prediction_vector in zip(range(labels.size), predictions.T):
    ...
    elif on_testing_data:#storing all images model got wrong on testing data
...
def test(iw, ib, ow, ob, test_images, test_labels):
  ...
  _, _, outputs = fwd_prop(iw, ib, ow, ob, test_images)

  print("\n\ntest sample accuracy: ", get_accuracy(outputs, test_labels, True))

  #returning transpose so images can be drawn correctly
  return outputs.T